...
public class EmailService {
    ...
    public void sendRecoveryEmail(String to, String token) {
                ...
                <div style="max-width: 460px; margin: 0 auto; background-color: #ffffff; border: 1px solid #e5e7eb; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.05);">
                    ...
                    <div style="border-top: 1px solid #f1f5f9; padding: 16px 24px; background-color: #fafafa; text-align: center;">
                        <p style="color: #94a3b8; font-size: 12px; margin: 0;">
                            &copy; %d Finder. Todos os direitos reservados.
                        </p>
                    </div>
                    
                </div>
            </div>
            """.formatted(token, Year.now().getValue());

        Map<String, Object> body = Map.of(
            "from", Map.of("email", "nao-responda@finder.blog.br", "name", "Finder"),
            "to", List.of(Map.of("email", to)),
            "subject", token + " é o seu código de recuperação",
            "html", html
        );
        ...
        try {
            ...
            System.out.println("Email de recuperação enviado via API para: " + to);
        } catch (Exception e) {
            System.out.println("Erro ao enviar email via API Mailtrap: " + e.getMessage());
